...
class GeoSmoothing:
    ...
    def _neighbour_weight_setting(self):
        """
        Description:
    ...
    def _spatial_lag_iterator(self):
    ...
    def _smoothing_rounds(self):
        GeoSmoothing._spatial_lag(self)
        GeoSmoothing._create_outputs(self)
        while self.iter_round < self.max_iterations:
            GeoSmoothing._spatial_lag_iterator(self)
            GeoSmoothing._create_outputs(self)
    ...
    def run(self):
        self.iter_rounds=0
        GeoSmoothing._neighbour_weight_setting(self)
        GeoSmoothing._smoothing_rounds(self)
        return self.gdf

    def create_log(self):
        info_log = {}
        #add dictonary of attributes 
        info_log['Knn_Used']=self.knn
        return info_log
        #add logic here to ouput a log of all attributes used
        #Output as a dictonary
