...
Dataset_1_Title = "Legacy more than New"
Dataset_2_Title = "Legacy less than New"
...
for col in df_1.columns:
    if len(df_1[col].value_counts())==1 or len(df_2[col].value_counts())==1:
...
if equalise_datasets == True:
    smallest_dataset_size = min(len(df_1),len(df_2))
    print(f'Before trimming: Dataset_1 had {len(df_1)} rows. Dataset_2 had {len(df_2)} rows.')
    df_1 = df_1.head(smallest_dataset_size)
    df_2 = df_2.head(smallest_dataset_size)
    print(f'After trimming: Dataset_1 had {len(df_1)} rows. Dataset_2 had {len(df_2)} rows.')
else:
    print(f' Dataset_1 has {len(df_1)} rows. Dataset_2 has {len(df_2)} rows. No equalising has been done.')
...
if len(numerical_columns_df1) - len(numerical_columns_df2) != 0:
    print('Different number of numerical columns detected between 2 datasets. Please check format of columns. Only columns found as numerical in both datasets shall be filled with -1 as missing.')

numerical_columns_in_both = [value for value in numerical_columns_df1 if value in numerical_columns_df2]

print('Numerical columns are:', numerical_columns_in_both)

#fill in missing numerical columns with -1
kstest_df_1[numerical_columns_in_both] = kstest_df_1[numerical_columns_in_both].fillna(-1)
kstest_df_2[numerical_columns_in_both] = kstest_df_2[numerical_columns_in_both].fillna(-1)
...
display(results_df.sort_values('Statistic', ascending=False))
